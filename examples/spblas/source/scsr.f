...
!          MKL_SCSRGEMV    MKL_SCSRSYMV  MKL_SCSRTRSV.
!
...
!
!       1. The code computes (L+D)'*S = F using MKL_SCSRMM where S is a known 5 by 2
!          matrix and then the code solves the system (L+D)'*X = F with the help of MKL_SCSRSM.
!          It's evident that X should be equal to S.
!
!       2. The code computes (U+I)'*S = F using MKL_SCSRMV where S is a vector
!          and then the code calls MKL_SCSRTRSV solves the system (U+I)'*X = F with the single right
...
!          It's evident that X should be equal to S.
!
!       4. The next step is the computation (U-U') S = F using MKL_SCSRMV where S is
!          a vector. It is easy to see that U-U' is a skew-symmetric matrix.
!
!       5. The next step is the computation (L+D+L') S = F using MKL_SCSRSYMV where S is
...
!
!       6. The next step is the computation A'* S = F using MKL_SCSRGEMV where S is
!          a vector.
!
...
!*******************************************************************************
          data values1/1.0, -2.0, 5.0, 4.0,  -4.0, 2.0, 7.0,  8.0, -5.0/
...
!*******************************************************************************
     ...
     &    5.D0, 4.D0, 3.D0, 2.D0, 1.D0/
          ...
          integer i, j
          print*
          print*, ' EXAMPLE PROGRAM FOR COMPRESSED SPARSE
     &       ROW FORMAT ROUTINES '
...
!*******************************************************************************
          print*
          print*, '     INPUT DATA FOR MKL_SCSRMM '
          ...
          print 104, ((sol(i,j),j=1,n),i=1,m)
           ...
           call mkl_scsrmm('t', m, n, m, alpha, 'tln',
     &       values, columns, rowIndex, rowindex(2), sol, m,
     &    beta, rhs,  m)
          print*
          print*, '     OUTPUT DATA FOR MKL_SCSRMM '
          print*, '     WITH TRIANGULAR MATRIX  '
          print 104, ((rhs(i,j),j=1,n),i=1,m)
          print 100
          print*, ' Solve triangular system with obtained '
          print*, ' right hand side  '
          call mkl_scsrsm('t', m, n, alpha, 'tln',
     &         values,  columns, rowIndex,
     &    rowindex(2), rhs, m, temp,  m)

          print*
          print*, '     OUTPUT DATA FOR MKL_SCSRSM '
...
!*******************************************************************************
          ...
          enddo
          print*
          print*, '     INPUT DATA FOR MKL_SCSRMV '
          print*, '     WITH TRIANGULAR MATRIX  '
          print 102, alpha, beta
          print 103, 't'
          print*, ' Input vector '
          print 105, (sol(i,1),i=1,m)
           ...
           call mkl_scsrmv('t', m, m, alpha, 'tuu',
     &           values, columns, pointerB, pointerE, sol, beta, rhs)
          print*
          print*, '     OUTPUT DATA FOR MKL_SCSRMV '
          print*, '     WITH TRIANGULAR MATRIX  '
          print 105, (rhs(i,1),i=1,m)
          print 100
          print*, ' Solve triangular system with obtained '
          print*, ' right hand side  '
          call mkl_scsrtrsv('u', 't', 'u', m,
     &           values, rowIndex, columns, rhs, temp)
          print*
          print*, '     OUTPUT DATA FOR MKL_SCSRTRSV '
          print*, '     WITH TRIANGULAR MATRIX  '
...
!*******************************************************************************
          ...
          print 105, (sol(i,2),i=1,m)
           ...
           call mkl_scsrmv('n', m, m, alpha, 'dun',
     ...
     &          rhs)
          ...
          print*, ' with the help of MKL_SCSRSV '


          call mkl_scsrsv('t', m, alpha, 'dun',
     &         values, columns, rowIndex, rowIndex(2),  rhs, temp)

          print*
          print*, '     OUTPUT DATA FOR MKL_SCSRSV '
          print*, '     WITH DIAGONAL MATRIX  '
          print 105, (temp(i,1),i=1,m)
          print 100
...
!*******************************************************************************
          ...
          print 105, (sol(i, 1),i=1,m)
           ...
           call mkl_scsrmv('n', m, m, alpha, 'au',
...
!    MKL_SCSRSYMV
! NOTE: The routine mkl_scsrsymv as well as the similar dense Level 2
! routine ssymv has a possibilty to extract the required triangle from the input
...
!  same output vector
!  call mkl_scsrsymv(uplo, m, values, rowIndex, columns, sol_vec, rhs_vec);
!  call mkl_scsrsymv(uplo, m, values1, rowIndex1, columns1, sol_vec, temp);
!
!  The test checks whether these two calls give the same output vector.
!*******************************************************************************
          print*
          print*, '     SYMMETRIC MATRIX-VECTOR MULTIPLY ROUTINE '
          print*, '     INPUT DATA FOR MKL_SCSRSYMV '
          print*, ' Input vector '
          print 105, (sol(i, 1),i=1,m)

           call mkl_scsrsymv('l', m,  values, rowIndex, columns,
     &           sol, rhs)
           call mkl_scsrsymv('l', m,  values1, rowIndex1, columns1,
     &           sol, temp)
          print*
          print*, '     SYMMETRIC MATRIX-VECTOR MULTIPLY '
          print*, '     MKL_SCSRSYMV CALLED TWO TIMES '
          do i=1, m
              print 104, rhs(i,1), temp(i,1)
          enddo
          print 100
!*******************************************************************************
! Task 6.   Obtain matrix-vector multiply A'*sol --> rhs with the help of MKL_SCSRGEMV
!
!*******************************************************************************
          print*
     ...
     &                 MATRICES '
          ...
          print 105, (sol(i, 1),i=1,m)
           ...
           call mkl_scsrgemv('t', m,  values, rowIndex, columns,
     &           sol, rhs)
          print*
          print*, '     OUTPUT DATA FOR MKL_SCSRGEMV '
          print*, '     MATRIX-VECTOR MULTIPLY ROUTINE FOR GENERAL
     &       MATRICES  '
          print 105, (rhs(i,1),i=1,m)
          print 100
...
!*******************************************************************************
! Task 7.  Obtain matrix-vector multiply T*sol --> rhs with the help of MKL_SCSRMV
...
!*******************************************************************************
          ...
          print 105, (sol(i, 1),i=1,mnew)
           call mkl_scsrmv('n', mnew, mnew, alpha, 'tln', values,
     &       columns,   pointerB, pointerE,  sol, beta, rhs)
          ...
          call mkl_scsrsv('n', mnew, alpha, 'tln', values,
