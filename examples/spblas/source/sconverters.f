...
c*******************************************************************************
     ...
     &            ldAbsr=4, nn=2, mn=16, idiag=3, ndiag=4)
      real ADNS(m,n),   ADNS_standard(m,n)
      real Absr(nzmax), Absr_standard(nzmax)
      real Acsc(nzmax), Acsc_standard(nzmax)
      real Acsr(nzmax), Acsr_standard(nzmax)
      real Acoo(nzmax), Acoo_standard(nzmax)
      real Adia(idiag*ndiag), Adia_standard(idiag*ndiag)
      real Asky(6)
      real Askyl_standard(6)
      ...
      data Absr_standard / 5.0, 9.0, 8.0, 2.0, 3.0,
     &                     1.0, 6.0, 4.0/
      data Acsc_standard / 5.0, 9.0, 8.0, 2.0, 3.0,
     &                     1.0, 6.0, 4.0/
      data Acsr_standard / 5.0, 8.0, 9.0, 2.0, 3.0,
     &                     6.0, 1.0, 4.0/
      data Acoo_standard / 5.0, 8.0, 9.0, 2.0, 3.0,
     &                     6.0, 1.0, 4.0/
      data Adia_standard / 0.0, 9.0, 0.0, 1.0,
     &                     5.0, 2.0, 3.0, 4.0,
     &                     8.0, 0.0, 6.0, 0.0/
      data Askyl_standard /5.0, 9.0, 2.0, 3.0, 1.0, 4.0/
...
c***************************************************************************************************
c TASK 1    Obtain sparse row format from densely matrix
c***************************************************************************************************
      do j=1,n
        do i=1,m
           ADNS(i,j)=0.0
      ...
      job(1)=0
      job(6)=1
      call mkl_sdnscsr(job,m,n,Adns,lda,Acsr,AJ,AI,info)
...
c***************************************************************************************************
      job(1)=0
      job(6)=3
      call mkl_scsrcoo (job,n, Acsr, AJ,AI,nnz,Acoo, ir,jc,info)
...
c***************************************************************************************************
      ...
      job(1) = 0
      job(6) = 1
      call mkl_scsrbsr (job,m,mblk,ldAbsr,Acsr,AJ,AI,Absr,AJB,AIB,info)
      nr = 1 + (m-1) / mblk
...
c***************************************************************************************************
      job(1) = 1
      job(6) = 3
      call mkl_scsrbsr (job,nn,mblk,ldAbsr,Acsr,AJ,AI,Absr,AJB,AIB,info)
...
c***************************************************************************************************
      ...
      id = idiag
      nd = ndiag
      call mkl_scsrdia(job,n,Acsr,AJ,AI,Adia,nd,distance,id,Acsr,AJ,
     &                 AI,info)
...
c***************************************************************************************************
      job(1) = 1
      job(6) = 0
      call mkl_scsrdia(job,n,Acsr,AJ,AI,Adia,nd,distance,id,Acsr,
     &                 AJ,AI, info)
...
c***************************************************************************************************
      job(1) = 0
      job(6) = 1
      call mkl_scsrcsc(job,n,Acsr,AJ,AI,Acsc,AJ1,AI1,info)
...
c***************************************************************************************************
      ...
      job(5) = 6
      job(6) = 0
      call mkl_scsrsky(job,n,Acsr,AJ,AI,Asky,pointers,info)
